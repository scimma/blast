...
There are three Docker volumes created for persistent data storage: (1) :code:`init-data`, (2) :code:`blast-db`, and (3) :code:`django-static`. When you run Blast for the first time, these are created automatically by Docker, and then the initialization script populates each volume.
...
The :code:`init-data` volume stores astronomical data required by calculations during the workflow. During initialization, all required data files are downloaded and installed.
...
Due to the size of the initial data files, the :code:`init-data` volume is shared between the 
