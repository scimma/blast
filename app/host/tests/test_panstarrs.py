...
class TestPanSTARRS(TestCase):
    ...
    def plot_crab_nebula(self):
        ...
        # grayscale image
        gim = self.getgrayim(ra, dec, size=size, filter="i")
        # color image
        cim = self.getcolorim(ra, dec, size=size, filters="grz")
    ...
    def test_download_fits(self, fits_filepath='/tmp/panstarrs_test.fits', ra=83.633210, dec=22.014460, size=1280):
        # %% [markdown]
        ...
        fitsurl = self.geturl(ra, dec, size=size, filters="i", format="fits")
        fh = fits.open(fitsurl[0])
        from host.object_store import ObjectStore
        s3 = ObjectStore()
        fh.writeto(fits_filepath, overwrite=True)
        print(f'"{fits_filepath}" md5sum: {s3.md5_checksum(fits_filepath)}')
        # fim = fh[0].data
